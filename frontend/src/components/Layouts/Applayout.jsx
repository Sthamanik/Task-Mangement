...
const Applayout = () => {
  ...
  const closeTaskpanel = () => setIsOpen(false);
    ...
    <>
      ...
      <div className='h-dvh w-dvw p-16 flex justify-center items-center bg-gradient-to-bl from-slate-500 via-indigo-400 to-cyan-700'>
        <div className='h-full w-full bg-white bg-opacity-25 backdrop-filter backdrop-blur-sm rounded-2xl shadow-xl shadow-indigo-800 overflow-hidden  flex relative'>
          ...
          <Routes>
            <Route exact path='/tasks/pending' element={<PendingTasks pendingTasks={pendingTasks} pendingCount={pendingCount} />} />
            <Route exact path='/tasks/completed' element={<Completed completedTasks={completedTasks} completedCount={completedCount} />} />
            <Route exact path='/tasks/archived' element={<Archived archivedTasks={archivedTasks} archivedCount={archivedCount} />} />
            <Route exact path="/profile" element={<Profile tasks={noOfTasks} />} />
          </Routes>
          <AddTasks isClosed={!isOpen} closeTaskpanel={closeTaskpanel} />
        </div>
      </div>
    </>
  );
